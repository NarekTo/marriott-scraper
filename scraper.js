...
let counter = 0;
let counterWorkerIndex = 0;
let counterWorkerChunk = 0;
...
async function scrapeAll(browserInstance) {
    ...
    for (j = counterWorkerChunk; j < options.numWorkers; j = counterWorkerChunk + number_of_urls / options.numWorkers) {
        try {
            writeStream
                .pipe(createGzip())
                .pipe(outStream);
            ...
            for (i = counterWorkerChunk; i <= number_of_urls; i++) {
                // counterWorker ++;
                {
                    try {
                        ...
                        const result =
                        {
                            outputFile: options.getS3OutputFilename(counterWorkerIndex),
                            counter: counter,
            ...
            console.log('ending');
            // console.log('counterWorker', counterWorker);
            writeStream.end();
            await browser.close();

            await s3Client.upload({
                Key: options.getS3OutputFilename(counterWorkerIndex),
